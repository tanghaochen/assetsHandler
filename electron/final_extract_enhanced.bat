...
:: ============================================
:: Enhanced ZIP Processing Script - Enhanced Version
:: 支持通过环境变量传递参数
:: ============================================

:: 从环境变量读取配置，如果没有设置则使用默认值
if not defined PASSWORD set "PASSWORD=3y@Ef!YzJNmY"
if not defined SUFFIX set "SUFFIX=_installguider.com"
if not defined COPY_FILE_PATH set "COPY_FILE_PATH=E:\TB\看更多软件资源.url"
if not defined COPY_FILE_ENABLED set "COPY_FILE_ENABLED=true"
if not defined DELETE_ORIGINAL set "DELETE_ORIGINAL=true"
if not defined EXTRACT_NESTED set "EXTRACT_NESTED=true"

set "SEVENZIP=7z"

echo ============================================
echo Enhanced ZIP Processing Script - Enhanced Version
echo Password: %PASSWORD%
echo Suffix: %SUFFIX%
echo Copy File Path: %COPY_FILE_PATH%
echo Copy File Enabled: %COPY_FILE_ENABLED%
echo Delete Original: %DELETE_ORIGINAL%
echo Extract Nested: %EXTRACT_NESTED%
echo ============================================
echo.
...
for %%F in (*.zip) do (
    ...
    :: Extract nested ZIP files recursively if enabled
    if "!EXTRACT_NESTED!"=="true" (
...
echo - Final archives created with %SUFFIX% suffix
